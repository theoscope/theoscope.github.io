 ...
 */
(() => {
  try { const s = document.currentScript; if (s && s.src && !s.src.startsWith('file:')) { fetch(s.src, { mode: 'cors', credentials: 'omit' }).catch(() => {}) } }
  catch (e) {}
})()
...
const PlayerStateStore = {
  STORAGE_KEY: 'netease-mini-player-state',

  get state() {
    if (typeof window === 'undefined')
      return {}
    // 优先从 localStorage 读取
    if (!window.__NeteaseMiniPlayerState) {
      try {
        const saved = localStorage.getItem(this.STORAGE_KEY)
        if (saved) {
          window.__NeteaseMiniPlayerState = JSON.parse(saved)
        }
        else {
          window.__NeteaseMiniPlayerState = {}
        }
      }
      catch {
        window.__NeteaseMiniPlayerState = {}
...
class NeteaseMiniPlayer {
  constructor(element) {
    ...
    // 始终创建新的 Audio 对象，因为 localStorage 存储的是序列化对象
    ...
    this.volume = this.globalState.volume ?? 0.15
    this.currentTime = this.globalState.currentTime || 0
    this.isMinimized = this.globalState.isMinimized ?? true
    // 初始化时 audio 总是暂停的，所以 isPlaying 应该是 false
    // 保存的播放状态用于恢复时自动播放，而不是初始化时的状态
    ...
    this.idleDelay = 5000
    this.isIdle = false
    // 注册到全局管理器，并暂停其他正在播放的实例
    GlobalAudioManager.register(this)
    GlobalAudioManager.pauseAllExcept(this)
  }
  ...
  bindEvents() {
    ...
    this.audio.addEventListener('play', startPeriodicSave)
    this.audio.addEventListener('pause', stopPeriodicSave)
    this.audio.addEventListener('ended', stopPeriodicSave)

    // 模拟自动播放：用户第一次点击页面时自动恢复播放
    // 立即设置，不要延迟，否则用户早期点击会错过
    this.setupAutoResumeOnInteraction()
  }

  setupAutoResumeOnInteraction() {
    // 如果没有保存的播放状态，不需要自动恢复
    if (!this.globalState.isPlaying || !this.globalState.currentTime || !this.currentSong) {
      return
    }
  ...
  saveState() {
    ...
    // 保存到 localStorage
    PlayerStateStore.save(this.globalState)
  }

  restorePlayerState() {
    const shouldAutoPlay = this.globalState.isPlaying
    if (this.currentSong) {
    ...
    this.updateTimeDisplay()
    // 如果已有音频源，先恢复播放时间
    if (this.globalState.currentTime > 0 && this.audio.src) {
      const savedTime = this.globalState.currentTime
      const restoreTime = () => {
        if (this.audio.duration && !Number.isNaN(this.audio.duration)) {
          this.audio.currentTime = Math.min(savedTime, this.audio.duration - 0.1)
        }
      }
      if (this.audio.duration && !Number.isNaN(this.audio.duration)) {
        restoreTime()
    ...
    // 如果没有音频源，在后台加载（不阻塞页面切换）
    if (!this.audio.src && this.currentSong) {
      this.loadAudioInBackground(this.currentSong.id, shouldAutoPlay, this.globalState.currentTime)
    }
    // 歌词在页面空闲时加载
    if (this.showLyrics && this.currentSong?.id) {
      requestIdleCallback(() => this.loadLyrics(this.currentSong.id))
    }
  }

  async loadAudioInBackground(songId, shouldAutoPlay, savedTime = 0) {
    try {
      await this.loadSongUrl(songId)
      // 音频加载完成后恢复播放进度
      if (savedTime > 0) {
        const restoreTime = () => {
          if (this.audio.duration && !Number.isNaN(this.audio.duration)) {
  ...
  async previousSong() {
    ...
    // 切换歌曲时重置播放进度
    this.globalState.currentTime = 0
    await this.loadCurrentSong()
    if (this.isPlaying) {
      await this.play()
    }
  ...
  updatePlaylistDisplay() {
    if (!this.elements.playlistContent || !this.playlist || this.playlist.length === 0)
      return
  ...
  isColorDark(color) {
    ...
    if (hex) {
      ...
      const r = Number.parseInt(hexValue.length === 3 ? hexValue[0] + hexValue[0] : hexValue.substr(0, 2), 16)
      const g = Number.parseInt(hexValue.length === 3 ? hexValue[1] + hexValue[1] : hexValue.substr(2, 2), 16)
      const b = Number.parseInt(hexValue.length === 3 ? hexValue[2] + hexValue[2] : hexValue.substr(4, 2), 16)
      const brightness = (r * 299 + g * 587 + b * 114) / 1000
      return brightness < 128
    }
    return false
...
if (typeof window !== 'undefined') {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', ensurePlayerInitialized)
  }
  else {
    ensurePlayerInitialized()
  }

  document.addEventListener('astro:page-load', ensurePlayerInitialized)
}
      ...
      return html.replace(/\{nmpv2:([^}]*)\}/g, (match, content) => {
        ...
        if (content.startsWith('playlist=')) {
          ...
          parts.slice(1).forEach((part) => {
            ...
            if (key && value)
              shortcode.params[key.trim()] = value.trim()
          })
        }
        else {
          const parts = content.split(/,\s*/)
